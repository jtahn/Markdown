...
- strategy
	...
	- once a combo is formed, ie the base/return case:
		- use an aux function `isValidParentheses` to check if it is well-formed
			- if so, store it
- complexity
	- time: O(2^m * m) where m = 2n
	- space: nth catalan number
		- bc we dont actually store every combo; we only store the ones that are well-formed
- observe
	- the 'backtracking' solution is more time efficient, bc you dont waste time from completely building out every non-well-formed combo
		- ie you either never build an unfinished product that has no chance to be well-formed
			- or you just immediately stop building once you realized that it has no chance to be well-formed
...
- what is the correct jargon here? so many things being thrown around
...
# (todo) References/optimization over backtracking
```
class Solution:
    def generateParenthesis(self, n: int) -> List[str]:
        stack = []
