...
- rewrite 'explanation', and maybe use this problem as an example in a blurb
	- (maybe this should be an example for the blurb i had earlier, about how the ‘structure’ isnt actually the key idea..its really, ‘what you want to store’)
	- it’s not about whether i use decreasing/increasing stack
	- it’s about:
		- step 1: what info do i want to store?
		- step 2: based on what i want to store: what structure should i use
		- (optional step 3): observe that the stack will be increasing/decreasing
			- also maybe observe: ‘maintaining the structure’ (ie via the inequality that defines whether you pop/push) becomes and equivalent condition/definition of ‘what i want to store’
...
- key idea 1
	- use a stack to do stuff in 1 pass
- key idea 2
	- bounded temp range allows for O(1) instead of O(n) space
	- this should be it’s own heading
	- figure out best wording for this
- trying to word key idea 2:
	- forward temps, forward iteration is like indices that require an update
- backwards iteration is indices that could force an update
- if it's warmer than stuff in the future, then you remove/pop that stuff
- bc the future stuff wouldn't force an update
- bc the more recent AND warmer temperature would have forced it already
...
- ‘array jumping’
	- is this same space complexity as stacks? if so, delete?


- discussion about ‘direction you iterate implies what you have to store’ is way too detailed
	- all that is needed: for each direction:
		- what would be useful to store / what are you storing/keeping track of
		- 
- imo this should be fundamental problem about how: ‘direction you iterate’ can have big implications
	- direction you iterate: kind of ‘forces’ what you need to store, bc you need to store info that is useful/actionable to future iterations
		- or at least, makes a difference in what you’re doing/storing; bc seeing things in different order
