...
- the 2 approaches (1 pass vs 2 pass) are really the same idea, so they should be same heading
	- make code cleaner + add comments; no ‘prose explanation’ needed
	- just say: general strategy is use the dictionary to keep track of the copied version of the original node
	- then two typical ways to implement this idea; see code
