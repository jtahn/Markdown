...
- cite fundy for ‘valid anagram’
	- move/remove most discussion, but keep code
	- ie 1 vs 2 hashmaps
		- implementing hashmaps
		- dict comparisons
		- optimization: variable to keep track of freqs / similar thicks
...
- mention/fundy? python slicing behavior
	- what is returned when you slice an iterable, but same start and end indices
		...
		- btw is there general ‘jargon’ for this? ie techniques that, incorporate a flag into variables you already use? iirc this is used in the ‘01 matrix’ problem iirc (ie using -1 to indicate, we havent checked this location yet)


- delete that screenie of guy not understanding why you enumerate from 1
	- this is just him not realizing that it’s to handle the ‘no solution’ case
